...
public class EX05 {
    public static void main(String[] args) throws IOException {
        ...
        w50000 = charge / 50000;
        //charge = charge% 50000; = charge %=  50000;
        charge %=  50000;
        ...
        switch (isMarried){
            case "y": case "Y":
                ...
                else tax = salary * 0.35; break;
            case "n": case "N":
                if (salary < 3000) tax = salary * 0.1;
                else tax = salary * 0.25; break;
        }
        ...
        //029- UpperCase
        // System.in.read(); :  키보드로 부터 문자 하나를 입력받음
        // 단, 입력받은 문자는 ASCII코드값(10진수) 이기 때문에 char형으로의 변환 필요!!
        //A의 ASCII코드값 : 65
        //a의 ASCII코드값 : 97
        // 소문자와 대문자의 차이 : 32
        ...
        if (lwch >= 97 && lwch <= 122) {
            ...
            result = String.format("입력한 %c의 대문자는 %c입니다.", lwch, upch);
        } else {
            result = "잘못입력하셨습니다.";
        }
        System.out.println(result);
        ...
        // 임의의 숫자 6자리를 입력하면
        // 신용카드의 종류와 은행정보 출력
        System.out.print("카드번호는? 예)404825");
        String num = sc.next();
       ...
       // 34. 거리에 따라 다음 요금이 정해져 있다. 거리를 km로 입력했을 때 배송료를 계산하는 프로그램을 작성하여라. (CalcurateFee)
//        가. 50km 미만 : 10000원
//        나. 50 ~ 100km : 18000원a
//        다. 100 ~ 300km : 55000원
//        라. 300km 이상 : 75000원
        System.out.println("배송료 계산");
        System.out.print("몇km 인가요?");
        int distance = sc.nextInt();

         if (distance >= 300) result = "75000원";
         else if (distance >= 100) result= "55000원";
         else if (distance >= 50) result= "18000원";
         else result= "10000원";
