...
public class J10Method {
    public static void main(String[] args) {
        ...
        int year = 2020;
        System.out.println( computLeapYear(year) );
    ...
    // 윤년여부를 계산하는 반환형(return) 메서드  : computeLeapYear
    public static String computLeapYear(int year){
        String result = "평년입니다.";
    ...
    public static String showAMPM(String time){
        String result = "오전입니다";
        int hour = Integer.parseInt(time.charAt(0) + "" +time.charAt(1));  //ascii코드
    ...
    public static void readSungJuk(String[] names, int[] kors, int[] engs, int[] mats){
        ...
        for (int i = 0; i < names.length ; i++) {
            System.out.print((i + 1) + "번 학생이름은??");
            names[i] = sc.next();
            System.out.print((i + 1) + "국어 점수?");
            kors[i] = sc.nextInt();
            System.out.print((i + 1) + "영어 점수?");
            engs[i] = sc.nextInt();
            System.out.print((i + 1) + "수학 점수?");
            mats[i] = sc.nextInt();
        }   //성적 데이터 입력
    }

    public static void computesSungjuk(int[] kors, int[] engs, int[] mats,
                                    int[] tots, double[] avgs, String[] grds){
   ...
   public static void printSungJuk(String[] names,int[] kors, int[] engs, int[] mats,
                 int[] tots, double[] avgs, String[] grds){
        String result = "";
        for (int i = 0; i < names.length; i++) {
            result += String.format("%s %d %d %d \n",names[i],kors[i],engs[i],mats[i]);
            result += String.format("%d %.1f %s \n",tots[i],avgs[i],grds[i]);
        }   // 결과 출력 준비
