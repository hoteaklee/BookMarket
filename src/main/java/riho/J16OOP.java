...
public class J16OOP {
    ...
    //클래스 형변환
    // 다형성의 또 다른 예
    // upcasting : 자식클래스가 부모 클래스로 변환하는 것을 의미 (자동)
    // downcasting : 부모클래스가 자식클래스로 변환하는 것을 의미 (수동)

    public static void main(String[] args) {
        //SCV scv1 = new SCV();
        Unit scv1 = new SCV();  // 다형성 : upcasting -> scv 가 unit 형으로 바꿈

        Marine marine1 = new Marine();
        Firebat firebat1 = new Firebat();
        Medic medic1 = new Medic();

        System.out.printf("SCV 생명력 : %d\n",scv1.life);
        scv1.attack();
        // scv1.collect();  //unit 클래스에서는 collect 메서드가 없음
        ((SCV) scv1).collect(); // 다형성 : downcasting
        // scv에는 collect 메서드가 있으므로 unit -> scv 로 변환

        System.out.printf("마린 생명력 : %d\n",marine1.life);
        marine1.attack();
        marine1.useStimpack();
        System.out.printf("파이어벳 생명력 : %d\n",firebat1.life);
        firebat1.attack();
        firebat1.useStimpack();
        System.out.printf("메딕 생명력 : %d\n",medic1.life);
        medic1.useHeal();
...
class SCV0{
    ...
    protected void collect(){ System.out.println("미레랄이나 개스핀 가스를 캐는중...");}
}
...
class SCV extends Unit{
    public SCV() {
        // super : 보무클래스의 생성자를 호출하는 특수한 키워드
        super(60, 5, 2.3444, 7, 20);
    }
    @Override   //@ 는 annotation주석 (컴파일러만 볼수 있는 메모)
    protected void attack() {
        System.out.printf("융합 절단기(%d)로 공격중...\n",power);
    }

    protected void collect(){
        System.out.println("미레랄이나 개스핀 가스를 캐는중...");
    }
...
class Marine extends Unit{
    ...
    protected void useStimpack(){
        System.out.printf("전투자극제 사용 : 공격력이 %.1f로 증가, 이동속도%.1f로 증가\n",power*1.72,move*1.5);
    }
}
