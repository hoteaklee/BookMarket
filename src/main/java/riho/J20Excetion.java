...
//        System.out.println("--------------------");
        ...
        try {
            ...
            System.out.println(">> 나누기 연사시 0은 사용불가!! <<");
            System.out.println(ex.getMessage());
        }
        ...
        try {
            ...
            for (String val : names){System.out.printf(" %s ",val);}
    ...
    // 비검사형 예외 : unchecked exception (컴파일타임 예외)
    ...
    // 단, 검사형 예외를 피하고 싶으면
    // throws를 이용해서 예외처리를 외부로 떠넘기면 죔 (비추!!)
    ...
    try {
        ...
        // 사용자 정의 예외는 Exxeption 클래스를 상속해서 만들어야 함
        // 예외처리 5
        ...
        try{
            try {
                ...
                throw  new ZeroDivideException("0으로 나룰수 없습니다.");
            }
        } catch (Exception ex) {     //사용자 정의 예외
                System.out.println(ex.getMessage());
        }
...
// 사용자 정의 예외 - 0으로 나울려고 할때 발생하는 예외에 대한 예외클래스
