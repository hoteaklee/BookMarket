...
public class J24File {
    public static void main(String[] args) {
        ...
        // 구성된 객체 데이터를  students.dat에저장

        // 학생 객체 생성
        Students std01 = new Students(
                201350050,"김태희","경기도 군포시",
                "1985.3.22","컴퓨터",504);
        ...
        try {
            ...
            fw.write(std01.toString()); // std01 객체 자체를 저장할수 없어 toString으로  해줌
        } catch (Exception ex){
        ...
        try {
            ...
            // 파일쓰기가 끝나면 작업 종료
            if (fr != null) try {br.close();} catch (Exception ex){}
            if (fr != null) try {fr.close();} catch (Exception ex){}
        }

        // 변수에 저장된 학생정보 출력 1
        System.out.print("변수에 저장된 학생정보 출력 1: ");
        System.out.println(stdinfo);

        // 변수에 저장된 학생정보 출력 1
        System.out.println("변수에 저장된 학생정보 출력 2: ");
        String[] info = stdinfo.split("[ ]"); // 뛰어쓰기 되어있는 순으로 분리해서 저장 배열에 저장

        System.out.println("학번 : " + info[0]);
        System.out.println("이름 : " + info[1]);
        System.out.println("주소 : " + info[2] + info[3]);
        System.out.println("생년 : " + info[4]);
...
class Students {
    ...
    public Students(int hakbun, String name, String addr, String birth, String dept, int prof) {
        this.hakbun = hakbun;
        this.name = name;
        this.addr = addr;
        this.birth = birth;
